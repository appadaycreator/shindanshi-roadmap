...
const SVC_URL = "https://appadaycreator.github.io/shindanshi-roadmap/";
const SVC_NAME = "中小企業診断士ロードマップ";

test.describe("中小企業診断士ロードマップ - E2Eテスト", () => {

  test("ページ基本: タイトル・h1・canonical", async ({ page }) => {
    await page.goto(SVC_URL);
    const title = await page.title();
    expect(title.length).toBeGreaterThan(15);
    expect(title.toLowerCase()).toMatch(/診断士/i);
    const canonical = await page.locator('link[rel="canonical"]').getAttribute("href");
    expect(canonical).toContain("shindanshi-roadmap");
  ...
  test("SEO: sitemap.xml・robots.txt が存在する", async ({ page }) => {
    const sitemapResp = await page.request.get("https://appadaycreator.github.io/shindanshi-roadmap/sitemap.xml");
    expect([200, 301, 302]).toContain(sitemapResp.status());
    const robotsResp = await page.request.get("https://appadaycreator.github.io/shindanshi-roadmap/robots.txt");
  ...
  test("プライバシー・利用規約: ページが存在する", async ({ page }) => {
    const privResp = await page.request.get("https://appadaycreator.github.io/shindanshi-roadmap/privacy-policy.html");
    const termsResp = await page.request.get("https://appadaycreator.github.io/shindanshi-roadmap/terms.html");
